...
func (h *API) scenarios(w http.ResponseWriter, r *http.Request) {
	...
	case "log":
		...
		if body.Status == "" {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": "укажите status"})
			return
		}
		body.Serial = serial
		body.ScenarioID = scenarioID
		if err := h.svc.AppendLogEntry(r.Context(), serial, scenarioID, body); err != nil {
			writeSvcErr(w, err)
			return
		}
		writeJSON(w, http.StatusOK, map[string]string{"message": "запись добавлена"})
	case "trigger":
